...
Hero: slide 0 = newest Continue entry (shares ONE detail fetch with resume-card[0] through a seeded `tmdbMetaCache` entry) with ▶ Resume, else trending #1; slides 1–4 trending; 7 s auto-advance paused on mouse hover only (audit A5), arrows + dots, stilled under reduced motion. Onboarding card replaces everything when there is no key AND no source (audit B8).
